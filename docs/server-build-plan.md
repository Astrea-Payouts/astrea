...
| --- | --- | --- | --- |
| K01 | Custom Soroban escrow contract spike: script-driven deploy → fund → approve → release → dispute → resolve, end-to-end on testnet. Validate the role model: judge acts as both approver and release signer; the funder cannot withdraw funded amounts once escrowed | L → split | **Gates the whole plan** — if the role model can't be built cleanly from scratch, everything downstream needs to change |
| K02 | Go ↔ Soroban integration spike: confirm a Go service can build, sign, and submit Stellar transactions against the K01 contract end-to-end (deploy/fund/approve/release) using the Stellar Go SDK | M | |
| K03 | Wallet compatibility check: confirm the contract works across the wallets the team plans to support (e.g. Freighter, Albedo, xBull, LOBSTR via Stellar Wallets Kit) | S | |
