...
On-chain trigger: the event's `judging_deadline` passes while it is still `InProgress` and the judge hasn't called `release_reward`. There is no separate "open a dispute" transaction — `resolve_dispute` is both the eligibility check (it asserts the deadline has already passed) and the resolution, in one resolver-signed call. **Before that deadline, a `LIVE`/`InProgress` event has no unwind path at all** — `set_event_cancelled` explicitly rejects anything past `Created`/`WaitingForStart`, and no other function can touch a live event's funds. So "the organizer wants to cancel a `LIVE` event" is not a distinct on-chain path — it can only actually be resolved through the same post-deadline `resolve_dispute` call as a silent judge, not on demand.
