...
Phased plan with coded tasks. Each task becomes one GitHub issue with its code in the title (e.g., `[E01] (contract) Escrow contract`). Sizes: S (≤half day), M (1–2 days), L (3+ days, should be split before assignment). `GFI` = good first issue candidate.

> **Note (2026-09-09):** the rows below in Phase 1 were written against an earlier, aspirational "multi-milestone" contract design that was never built — a design where `create_event` took a list of independently payable prizes. The shipped contract reserves one `reward: i128` per event and pays it out to `1..N` winners in a single atomic `release_reward` call. Rows below were corrected to match; see the issue-tracker audit for the full list, and `smart-contracts/astrea/contracts/event-escrow/src/lib.rs` for the real signatures.
...
| K06 | ✅ **Done (2026-08)** — Multi-release resource budget spike: can `release_reward()` pay N winners (one token transfer each) in a single call without hitting Stellar Mainnet's per-invocation resource limits? | S | Confirmed clean up to 25 winners in one call (~1.2% of the instruction budget, `MAX_WINNERS = 25` in `lib.rs`) — no separate contract needed for category/multi-winner events, see ADR-002/ADR-006. Findings in [spikes/k06-multi-release-budget](../spikes/k06-multi-release-budget/README.md) |

## Phase 1 — Production contract

| Code | Task | Size | Notes |
| --- | --- | --- | --- |
| E01 | Escrow contract: a single shared contract holding a per-organizer balance (`AdminWallet`, ADR-006) instead of K01's one-contract-per-event shape. Organizer deposits/withdraws against their own balance; `create_event` reserves one `reward: i128` from it (ADR-002 — not a list of prizes; the per-winner breakdown is a product-level concept persisted off-chain, decided 2026-09-09, not an on-chain structure); the pre-launch cancel path (shipped as `set_event_cancelled`) and `release_reward` (pays 1..N winners, validated by K06) plus dispute/resolve-dispute round it out | L → split | Lives in `smart-contracts/astrea/contracts/event-escrow`. K01 proved the role model; this POC (deposit/withdraw/create_event already working) is the production shape's actual starting point, not K01's contract-per-event design |
| E02 | Contract unit + integration test suite: role-model checks (organizer cannot move funds, judge is the sole `release_signer` — no separate approver role), every negative path (unauthorized release, double release, release before the event reaches `InProgress`, dispute blocks release), event independence (one event's dispute has no effect on any other event's `reward`) | M | `soroban-sdk` testutils, in-process — fast, no network. Expand K01's 8 tests to cover the shared-contract, per-organizer-balance paths |
...
**Timeline, end to end.** The SCF Build process itself typically runs **3–6 months** across the interest-form → award cycle; the current round, [SCF #46](https://communityfund.stellar.org/awards/recxrSMYwAl8vcglg) (checked 2026-09-06), closes Build Submission 2026-11-08, with Panel Review Nov 11–18 and Community Vote Nov 23–Dec 3 — real dates, not the estimate this section used to carry. Audit Bank intake and readiness assessment then takes **1–4 weeks** before the audit begins, plus the audit and the 20-day remediation window. Realistically, **from SCF application to an audited contract is on the order of half a year**, so mainnet planning should start from that number rather than from when the code feels finished.
