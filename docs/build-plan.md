...
| U17 | Optional organizer-defined registration questions: a toggle in U01's "Participantes" step, off by default (one-click registration stays the default). If enabled, the organizer adds simple custom fields (text / single-select / yes-no) that a participant fills at registration | S | Depends on U01, U04. Not a general form-builder — keep the field types minimal |
