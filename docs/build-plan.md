...
> **Note (2026-09-09):** several rows below were written against an earlier, aspirational contract design (multi-milestone events, a two-step "approve, then release" signing flow) that was never built. The shipped `event-escrow` contract reserves one `reward: i128` per event and pays it out in a single atomic `release_reward` call — see [contracts-build-plan.md](contracts-build-plan.md) for the real signatures. Rows below were corrected to match; see the issue-tracker audit for the full list.
...
| --- | --- | --- | --- |
| K02 | ✅ **Done (2026-08-06)** — Go ↔ escrow contract integration spike: confirmed a Go service can build, sign, and submit Stellar transactions against the contract end-to-end (deploy/fund/release) using the Stellar Go SDK | M | Findings in [spikes/k02-go-soroban](../spikes/k02-go-soroban/README.md) — no high-level "invoke" helper in the Go SDK, simulate→sign→submit→poll had to be hand-rolled |
...
| S02 | CI: GitHub Actions — build/lint/test for `apps/web` (already running) and `services/core-go` (`go build`/`go vet`/`go test`, not yet added) | M | One pipeline, every job runs unconditionally — no path-scoping. GitHub counts a skipped required check as not-passing, which already blocked every `develop` → `main` promotion once via the `commitlint` job; the `contracts` job carries a comment on this precedent |
...
| E03 | Event + prize state machine, with the manual-start rule built in from the start: `draft → CREATED → LIVE → JUDGING → COMPLETED`, plus `DISPUTED`/`CANCELLED` side-states reachable from `CREATED`, `LIVE`, or `JUDGING`. `draft` is client-side only (the wizard's autosaved draft, U01), before `create_event` is ever called. Track `conditionsMetAt` (derived: prize wallet funded **and** minimum participants registered) as distinct from `status = LIVE` — the event **never** auto-activates on conditions being met; activation only happens via an explicit admin action (`POST /events/:id/start`). Guard every transition against races (conditional update keyed on expected `from` status) | M | Building this in from day one is simpler than retrofitting it later. State names decided 2026-09-09 — reconciles what used to be two conflicting proposals across sibling issues |
...
| E07 | ✅ **Done** — Vertical slice demo: create event → fund → register participants → admin starts event → track in real time → assign winners → release payout (`release_reward`, a single atomic call — no separate on-chain "approve" step) → reconcile, on real testnet | M | Milestone: **the product's core guarantee works**, verified end-to-end before building UI polish on top. Run on production (2026-09-16, testnet): web on Vercel, Go on Vercel, Supabase Postgres. Event `83b1ea05-fb37-461f-b34f-aa99f83cae11`, escrow id `6e2b39904bb2a30cd8e70e5c1210b3e9`. Tx hashes — deposit 1 USDC `3f3f949ba40b5966feb474b14efad774045994f599ebb4ac43f72733f5a6a0a2`, `create_event` `aa73f9be73f05766b2ec1e52f4915099400ec3c605072ffd512df0dafe1fbe9a`, fee deposit `7eb8d0119eb13b1f0b2266b8303c8bfea27a6e2bed10ef9595d9e2e2d74e496a`, `start_event` `b84ad889ada3b806991d7b00e3f5ee41834d330b43fbd6c472963fd6c627cda7`, `release_reward` `79c27ffa0abe6a1c51633f01188184324d8f2785b4da917689b8c2b3d4812cd5` (1 USDC credited to the winning team's wallet). Gaps found during the run: the public event page does not show the payout tx hash (only the judge panel does); a failed submit leaves a `FAILED` build that "Try again" resubmits (`409 no_pending_*`) |
...
| L01 | ✅ **Done** — Deploy `apps/web` (Vercel or similar) and `services/core-go` (Docker + Railway/Fly.io/Render or a VPS); seed a standing demo event | S | The escrow contract deploys to Stellar testnet directly, not to the same host. Both services on Vercel: `apps/web` at astrea-payouts.vercel.app, `services/core-go` at astrea-core-go.vercel.app (Go framework preset, Root Directory `services/core-go`, production branch `main`). Postgres on Supabase via the transaction pooler — pgx needs `default_query_exec_mode=describe_exec`. Standing demo event still pending: the only production event so far is the completed E07 run |
...
| U05 | Judge panel: scoring/progress review, winner assignment (assigning a wallet to each prize position U01 declared — the judge doesn't set amounts), release signing | M | Single `release_reward` transaction — there's no separate on-chain "approve" step |
