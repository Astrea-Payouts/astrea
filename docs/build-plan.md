...
| L01 | ✅ **Done (2026-07-26)** — Deployed to Vercel, git-linked to `main` (auto-deploys on push/PR) at [astrea-payouts.vercel.app](https://astrea-payouts.vercel.app); seeded a standing demo event via `npm run demo:seed` | S | See `scripts/seed-demo-event.ts`. Event is funded and **LIVE, awaiting judging on purpose** (not run to completion like E06) — contract `CBE67YFSQIBXJFMCRJXQ7VPJJCYBDCG4JMDR35FA2YMQODBFYD4LPXBD`, so L02's demo video can record assign → approve → release → forward live instead of showing something that already happened. No public event page exists yet (that's U03), so the demo event isn't visible on the site itself yet — it's evidence for L02/L05, not a live homepage feature |
| L02 | ⏸ **Paused (2026-07-26)** — Demo video of the full flow | S | Checked GrantFox's actual "Create Your Project" form ([docs.grantfox.xyz](https://docs.grantfox.xyz/user-manual-guides/oss-contributions-guide/maintainer-guide/create-your-project)) — it's text fields (purpose, current state/traction, what contributions you want, why it's a good fit), no video field. Not worth recording today: there's no UI yet (Phase 3), so a video today would just be a terminal running `demo:e06`/`demo:seed`. Revisit once U01-U05 land and there's real UI to show |
| L05 | 🔜 **In progress** — GrantFox maintainer application: sync repo, complete project form | S | The point of it all. Application copy drafted (purpose/background, current state & traction citing K01/E06/L01's real testnet evidence, desired contributions, fit) — connecting GitHub and submitting the form on grantfox.xyz is a manual step only the maintainer can do |
