...
| Resolver (event.resolver) | Co-authorizes emergency_withdraw together with the organizer — dual authorization, both require_auth calls present. Also the sole caller of resolve_dispute, but only after event.judging_deadline passes without a release (section 6). | **Unilaterally move funds.** emergency_withdraw needs the organizer's signature too, so a compromised resolver alone drains nothing. Call resolve_dispute before the judging deadline. |
