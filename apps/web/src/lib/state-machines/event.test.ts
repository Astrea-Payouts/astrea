...
describe("event state machine", () => {
	it("allows the documented happy path", () => {
		...
		expect(canTransitionEvent("CREATED", "LIVE")).toBe(true);
	...
	it("can only reach LIVE from CREATED (create_event already funded it)", () => {
		expect(canTransitionEvent("DRAFT", "LIVE")).toBe(false);
		expect(canTransitionEvent("CREATED", "LIVE")).toBe(true);
		expect(canTransitionEvent("DISPUTED", "LIVE")).toBe(false);
