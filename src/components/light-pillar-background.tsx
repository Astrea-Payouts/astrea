...
// Adapted from React Bits' LightPillar (reactbits.dev/backgrounds/light-pillar),
// ported from its verified live source — see docs/ui-motion.md for why this
// replaced Prism for the hero. Note it has no built-in suspendWhenOffscreen
// (unlike Prism) — ui-motion.md already flagged this trade-off before the swap.
...
export function LightPillarBackground({
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: only re-init on webGLSupported/quality, matching the verified source — the other props are applied via the dedicated uniform-update effects below
	useEffect(() => {
		...
		const qualitySettings: Record<"low" | "medium" | "high", QualitySettings> =
			{
				low: {
					iterations: 24,
					waveIterations: 1,
	...
	useEffect(() => {
		...
		materialRef.current.uniforms.uTopColor.value = new THREE.Color(topColor);
	}, [topColor]);

	useEffect(() => {
		if (!materialRef.current) return;
		materialRef.current.uniforms.uBottomColor.value = new THREE.Color(
			bottomColor,
		);
	}, [bottomColor]);
	...
	if (!webGLSupported) {
		return (
			<div
				className={`absolute inset-0 flex items-center justify-center bg-black/10 text-sm text-neutral-500 ${className}`}
				style={{
